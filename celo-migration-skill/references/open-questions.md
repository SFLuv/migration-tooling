...
- ~~Will Berachain and Celo Ponder data live in separate databases or one migrated schema?~~ **Decided 2026-06-12: the Celo Ponder instance gets its own dedicated database (`MIGRATION_DB_CELO_PONDER_SUFFIX`). After it boots and goes live, `backfill-bera-history.sh` copies the normalized Berachain history into it, making the Celo DB the single cross-chain continuity ledger. Celo Ponder starts from `celo_distribution_complete_block + 1`.**
- Pause verification before the migration run is a manual operator step (decided 2026-06-12): confirm the Berachain token is locked and Berachain Ponder has indexed through the final block before running `run-migration.sh`; the script does not gate on it.
- **Confirmed constraint (2026-06-12, verified against ponder 0.15.18 source):** `ponder start` refuses a schema whose `_ponder_meta.build_id` differs from the current build, and the build id hashes the resolved config (contract address, start block, chain), so pointing at the Celo token guarantees a new build id. Deleting `_ponder_meta` does not help (table creation then fails on "already exists"), and if the previous app row has `is_dev = 1` the next start silently drops every table. The reuse plan therefore needs a decided, rehearsed path before cutover: either run Celo Ponder in a new schema (`DATABASE_SCHEMA`) in the same database with backend continuity reads spanning both schemas, or perform documented `_ponder_meta`/`_ponder_checkpoint` surgery (set `build_id`/`table_names` to the new build and delete stale Berachain checkpoint rows; with zero checkpoint rows Ponder skips crash recovery). Also: never restart the old Berachain Ponder after in-place normalization — same-build crash recovery replays stale 18-decimal `_reorg__*` rows onto normalized tables.
