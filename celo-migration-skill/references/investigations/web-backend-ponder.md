...
- The local migration tmux harness now starts by silently checking for any stale managed tmux session or local `migration_app`, `migration_bot`, and `migration_ponder` databases, only logging the startup cleanup phase when cleanup is actually needed, checks that all configured server ports are available, clones production app/bot/Ponder state from the root `PRODUCTION_POSTGRES_CONNECTION_STRING` into the root `LOCAL_POSTGRES_CONNECTION_STRING`, then starts and verifies Ponder before starting the backend. The backend pane still waits for the Ponder port before booting, so startup hook registration targets the intended local Ponder process. When the controller exits or receives Ctrl-C, it kills the tmux session and drops the three local migration databases.
- The harness restores cloned DBs through plain SQL, filters the harmless `SET transaction_timeout = 0;` emitted by newer `pg_dump` versions so local Postgres instances older than that setting can still restore production state, suppresses normal `psql` query output, and shows sparse clone progress checkpoints instead of an animated spinner because some IDE terminals append spinner redraws as log rows. The controller does not auto-attach to tmux panes unless run with `--attach`; by default it prints pane access commands and remains in the foreground so Ctrl-C can shut everything down. Dependency preflight checks for the local Ponder and frontend pnpm binaries before cloning DBs. Boot verification uses bounded `nc` probes, starts Ponder with `pnpm exec ponder start -H 0.0.0.0 -p $PONDER_PORT` by default, reports waiting progress, prints the relevant pane tail on failure, and triggers full shutdown if any service does not come up.
